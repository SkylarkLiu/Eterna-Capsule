  ...
  <view class="capsule-page">
    ...
    <view class="stats-row">
      ...
      <view class="stat-item glass-panel">
        ...
        <text class="stat-label">激活</text>
      </view>
      <view class="stat-item glass-panel">
        <text class="stat-value font-mono">0</text>
    ...
    <scroll-view class="capsule-list" scroll-y>
      ...
      >
        <view class="card-glow" :class="capsule.status"></view>
        <view class="card-content">
          <view class="card-header">
            <view class="capsule-icon-box" :class="capsule.status">
              <text class="capsule-icon">{{ capsule.icon }}</text>
            </view>
            <view class="capsule-info">
              <text class="capsule-name font-mono">{{ capsule.name }}</text>
              <text class="capsule-recipient">{{ capsule.recipient }}</text>
            </view>
            <view class="status-dot" :class="capsule.status"></view>
          </view>
          
          <view class="card-body">
            <view class="trigger-info">
              <text class="trigger-label">触发条件</text>
              <text class="trigger-value">{{ capsule.trigger }}</text>
            </view>
          </view>
          ...
          <view class="card-footer">
            <view class="encrypt-badge">
              ...
              <text class="encrypt-text">已加密</text>
            </view>
            <text class="created-date font-mono">{{ capsule.createdAt }}</text>
          </view>
        </view>

        <view class="particles" v-if="capsule.status === 'active'">
...
import type { Capsule } from '@/stores/capsule'
...
const viewCapsule = (capsule: Capsule) => {
  uni.showModal({
    title: capsule.name,
    content: `接收人: ${capsule.recipient}\n触发条件: ${capsule.trigger}\n状态: ${capsule.status === 'active' ? '激活中' : capsule.status === 'pending' ? '待激活' : '已投递'}`,
    showCancel: false
  })
}
</script>
...
.card-glow.pending {
  background: linear-gradient(135deg, rgba(0, 242, 255, 0.3) 0%, rgba(255, 92, 0, 0.3) 100%);
}
